...
uint32_t sensor_processing_from_storage(void)
{

    uint32_t temp;
    // A separate thread updates the temperature in this slot
    bpf_store_fetch_global(TEMP_NEW_DATA, &temp);

    // Update the temperature storage
    uint32_t pointer = 0;
    bpf_store_fetch_global(TEMP_DATA_PTR, &pointer);
    bpf_store_update_global(TEMP_DATA_PTR, (pointer + 1) % TEMP_STORAGE_SLOTS);
    bpf_store_update_global(TEMP_DATA_START + pointer, temp);

    for (uint32_t i = 1; i < TEMP_STORAGE_SLOTS; i++) {
        uint32_t old_temp = 0;
        uint32_t offset = (pointer + i) % TEMP_STORAGE_SLOTS;
        bpf_store_fetch_global(TEMP_DATA_START + offset, &old_temp);
        // We need to fill in empty values
        if (old_temp == 0) {
            bpf_store_update_global(TEMP_DATA_START + offset, temp);
        }
    }
    ...
    for (uint32_t i = 0; i < TEMP_STORAGE_SLOTS; i++) {
        uint32_t reading = 0;
        bpf_store_fetch_global(TEMP_DATA_START + i, &reading);
